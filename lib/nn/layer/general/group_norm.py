...
Modified by Torben Meyer on 8/10/2018 to support partially known shapes
...
def group_norm(inputs,
    ...
    axes_before_channels = tf.shape(inputs)[:channels_axis]
    axes_after_channels = tf.shape(inputs)[channels_axis+1:]

    # Manually broadcast the parameters to conform to the number of groups.
    params_shape_broadcast = ([1] * int(axes_before_channels.shape[0]) +
                              [groups, channels // groups] +
                              [1] * int(axes_after_channels.shape[0]))
    # Reshape the input by the group within the channel dimension.

    # inputs_shape = (axes_before_channels + [groups, channels // groups] +
    #                 axes_after_channels)
    channel_groups_shape = tf.constant([groups, channels // groups])
    inputs_shape = tf.concat(
        [axes_before_channels, channel_groups_shape, axes_after_channels], -1)
