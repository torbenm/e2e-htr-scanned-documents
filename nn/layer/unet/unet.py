...
def create_conv_net(x, dropout, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2,
    ...
    for layer in range(0, layers):
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            conv1 = conv2d(in_node, w1, b1, dropout, is_train, padding)
            tmp_h_conv = tf.nn.relu(conv1)
            conv2 = conv2d(tmp_h_conv, w2, b2, dropout, is_train, padding)
    ...
    for layer in range(layers - 2, -1, -1):
        with tf.name_scope("up_conv_{}".format(str(layer))):
            ...
            conv1 = conv2d(h_deconv_concat, w1, b1, dropout, is_train, padding)
            h_conv = tf.nn.relu(conv1)
            conv2 = conv2d(h_conv, w2, b2, dropout, is_train, padding)
    ...
    with tf.name_scope("output_map"):
        ...
        conv = conv2d(in_node, weight, bias, 0, is_train, padding)
